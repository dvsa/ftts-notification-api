...
describe('DeadLetterQueueCollector', () => {
  ...
  describe('processMessage', () => {
    test('emit event FOUND_MSG', async () => {
      ...
      expect(mockedLogger.logEvent).toHaveBeenCalledWith(mockedContext, BusinessTelemetryEvent.NOTIF_COLLECTOR_FOUND_MSG);
    });
    ...
    test('handles error from CRM', async () => {
      ...
      expect(mockedLogger.error).toHaveBeenCalledWith(
        crmError,
        'Could not process DLQ message, Error sending to CRM queue',
        expect.objectContaining({}),
      );
      expect(mockedLogger.logEvent).toHaveBeenCalledWith(mockedContext, BusinessTelemetryEvent.NOTIF_COLLECTOR_FAIL_CDS, expect.any(String));
    });
