...
describe('Dependency Injector Loader', () => {
  ...
  test('Ensure utilities are loaded', () => {
    ...
    expect(Container.set).toHaveBeenCalledWith('logger', logger);
    expect(Container.set).toHaveBeenCalledWith('validator:queuerecord', new QueueRecordValidator());
    expect(Container.set).toHaveBeenCalledWith('notify:controller', new NotifyController(mockedContext));
    expect(Container.set).toHaveBeenCalledWith('crm', expect.objectContaining({}));
  });
