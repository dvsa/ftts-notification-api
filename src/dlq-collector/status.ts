...
export const serviceBusQueueTrigger: AzureFunction = async (context: Context, message: QueueRecord): Promise<void> => {
  logger.event('Launch', 'Notifications Status DLQ Collector', { context });
  loaders(context);
  logger.logDependency(context, 'Ntf Status DLQ', 'Collecting message from Status DLQ', {
    dependencyTypeName: 'AMQP',
    id: context.traceContext.traceparent,
  });
  logger.info('Collecting Status DLQ message', { ...getIdentifiers(message), context });
  if (message) {
    const collector = new DeadLetterQueueCollector();
    await collector.processMessage(context, message);
  } else {
    logger.info('Status DLQ collector - service bus message does not exist', { ...getIdentifiers(message), context });
  }
};
